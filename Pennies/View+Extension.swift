...
extension View {
    ...
    func total(_ transactions: [Transaction], transactionCategory: TransactionCategory) -> Double {
        return transactions.lazy.filter({ $0.transactionCategory == transactionCategory.rawValue }).lazy.reduce(Double.zero) { partialResult, transaction in
